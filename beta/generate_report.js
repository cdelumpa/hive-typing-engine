...
// without a separate beta/package.json.
const path = require('path');
const ROOT = path.resolve(__dirname, '..');
const APP_MODULES = path.join(ROOT, 'app/node_modules');

require(path.join(APP_MODULES, 'dotenv')).config({ path: path.join(ROOT, 'app/.env') });

const fs   = require('fs');
const { Pool } = require(path.join(APP_MODULES, 'pg'));

const { buildBetaHTML, buildPdfOptions } = require(path.join(ROOT, 'app/renderer'));
