...
- **Phase 7 — Hardening.** **FIRST: wire the live server PDF path to the new reports.** `generateReportPDFs` (the `/api/submit` → email path in `server.js`) still calls the **V1** `renderer.buildClientHTML`/`buildCoachHTML` (A4). Switch it to the new pipeline — `report_prep.buildClientModel`/`buildCoachModel` → `renderer.buildClientReportHTML`/`buildCoachReportHTML` (Letter) → `measure.measureReport` gate → PDF — fetching the `client`/`coach` records from the DB. (Surfaced in Phase 6b: deleting the dead browser duplicate did NOT change the live path, which is still V1.) Then: automated regeneration loop on measurement failure; re-add the render half to `tests/run_test.js`; rewrite `beta/` + `app/generate_report.js`. *Verify:* end-to-end sp4/sx7 green through HTML + the live email path renders the new Part B/C reports.
